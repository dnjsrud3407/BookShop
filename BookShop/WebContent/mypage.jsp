...
function openWin(orderNum){  
    ...
    window.open("OrderExchange.mo?orderNum="+orderNum, "반품", "width=720, height=600, toolbar=no, menubar=no, scrollbars=no, resizable=yes" );  
}  

function openWin2(orderNum){  

    window.open("OrderRefund.mo?orderNum="+orderNum, "교환", "width=720, height=600, toolbar=no, menubar=no, scrollbars=no, resizable=yes" );  
}  
...
function openWin4(orderNum){  

    window.open("OrderCancel.mo?orderNum="+orderNum, "취소", "width=720, height=550, toolbar=no, menubar=no, scrollbars=no, resizable=yes" );  
}  
	...
	<c:forEach var="list" items="${orderList}" varStatus="status">
			...
			<table class="table table-bordered" style="vertical-align: middle;">
			 <c:if test="${orderNum != list.orderNum}">
              <thead>
                <tr>
                  <th colspan="3" id="orderTable">주문번호  ${list.orderNum}
...
<%-- 		  			 	</c:forEach> --%>
                  ...
                  <th id="orderTable" colspan="2">상태</th>
				</tr>
              </thead> 
              <tbody>
...
<!--ㅡㅡ--------------ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ------------------------------------------------------    -->
            ...
            <td style="border-bottom: none;">
            	 <c:forEach var="list2" items="${orderList2}" varStatus="status"> 
                   		 <c:if test="${list2.orderNum eq list.orderNum}">
                   		  <c:if test="${list2.pointAction==0}">
                   	   		<span class="label  label-info">사용포인트</span>	 ${list2.pointValue}원
          			  <c:set value="${list2.pointValue}" var="pointValue"></c:set>
                   	    </c:if>
                   	 	</c:if>
	  			 </c:forEach>
				 ...
				 <c:forEach var="couponList" items="${couponList}" varStatus="status">
                  		 <c:if test="${couponList.orderNum eq list.orderNum}">
                  		  <c:if test="${couponList.couponAction==1}">
                  	   			<span class="label label-success">사용쿠폰</span>  ${couponList.volume}원
                  	    </c:if>
                  	 	</c:if>
  			 	</c:forEach>
            </td>
            <c:set value="${list.orderNum}" var="orderNum"></c:set>
            <td style="border-bottom: none;" colspan="2"><c:if test="${list.orderStatus eq '배송완료'}"><div class="btn2" ><a href="OrderConFirmPro.mo?orderNum=${list.orderNum}" >구매확정</a></div></c:if></td>
            </tr>
            </c:if>

                  <td><img width="80px" height="100px" src="boardFile/${list.bookOriginImage}" alt=""/></td>
                  <td style="border-left: none;">상품명:${list.bookTitle}<br>번호:${list.orderNum}<br>주문일시:${list.orderTime}</td>
                ...
                <c:if test="${list.orderStatus eq '배송중' or list.orderStatus eq '배송완료'}"><div class="btn2"><a href="" onClick="javascript:openWin3(${list.orderDetailCode})">배송조회</a></div></c:if>  
                <c:if test="${list.orderStatus eq '결제완료'}"><div class="btn2"><a href="" onClick="javascript:openWin4(${list.orderDetailCode})">취소신청</a></div></c:if>
                <c:if test="${list.orderStatus eq '배송완료'}"><div class="btn2"><a href="" onClick="javascript:openWin(${list.orderDetailCode})">반품신청</a></div></c:if>
                <c:if test="${list.orderStatus eq '배송완료'}"><div class="btn2"><a href="" onClick="javascript:openWin2(${list.orderDetailCode})">교환신청</a></div></c:if>  
                <c:if test="${list.orderStatus eq '구매확정'}"><div class="btn2"><a href="ReviewWriteForm.book" id="orderConFrim">상품후기쓰기</a></div></c:if>  
                  </td>
                 ...
                 <div class="btn2" style="background-color: #9988;">${list.orderStatus}</div>
                  ...
                  <c:if test="${list.orderStatus eq '반품요청'}"><div class="btn2">반품처리중</div> </c:if>   
                  <c:if test="${list.orderStatus eq '교환요청'}"><div class="btn2">교환처리중</div> </c:if>
                  <c:if test="${list.orderStatus eq '취소요청'}"><div class="btn2">결제취소처리중</div> </c:if>    
                 </td>
                </tr>
