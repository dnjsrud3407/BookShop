...
<%--               	<c:set var="총금액" value=0 /> --%>
<%-- 				<c:foreach var="list" items="아이템"> --%>
<%-- 				<c:set var="total" value=${total + list.cart_total} /> --%>
<%-- 				</c:foreach> --%>
<%-- 				${총금액} --%>
					...
					<td>
	    			    ...
	    			    <select name="orderStatus" class="size" id="orderStatus">
		    			    <option value="<%=orderDetaile.getOrderStatus() %>" class="size" <c:if test="${orderDetaile.orderStatus eq '<%=orderDetaile.getOrderStatus() %>' }">selected</c:if>><%=orderDetaile.getOrderStatus() %></option>				
		    				    <option value="배송중" <c:if test="${orderDetaile.orderStatus eq '결제완료' }">selected</c:if>>배송중 </option>
		    				    <option value="배송완료" <c:if test="${orderDetaile.orderStatus eq '결제완료' }">selected</c:if>>배송완료</option>
