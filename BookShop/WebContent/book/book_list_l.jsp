		  ...
		  <label class="control-label alignL">자세히 </label>
			<select onchange="if(this.value) location.href=(this.value);">
			  <c:forEach var="bookK" items="${bookKategorie }" varStatus="status">
                <option value="BookList.book?type=L&bk2=${bk2 }&BKID=${bookK.bookKategorie_BKID}" 
...
<c:choose>
	<c:when test="${book.bookKategorie_BKID == 0 }">
		<div id="myTab" class="pull-right">
		 <a href="BookList.book?type=L&page=${pageInfo.page }&bk2=${bk2 }"><span class="btn btn-large btn-primary"><i class="icon-list"></i></span></a>
		 <a href="BookList.book?type=I&page=${pageInfo.page }&bk2=${bk2 }"><span class="btn btn-large"><i class="icon-th-large"></i></span></a>
		</div>
	</c:when>
	<c:otherwise>
		<div id="myTab" class="pull-right">
		 <a href="BookList.book?type=L&page=${pageInfo.page }&bk2=${bk2 }&BKID=${book.bookKategorie_BKID}"><span class="btn btn-large btn-primary"><i class="icon-list"></i></span></a>
		 <a href="BookList.book?type=I&page=${pageInfo.page }&bk2=${bk2 }&BKID=${book.bookKategorie_BKID}"><span class="btn btn-large"><i class="icon-th-large"></i></span></a>
		</div>
	</c:otherwise>
</c:choose>
<br class="clr"/>
...
<c:choose>
	<c:when test="${book.bookKategorie_BKID == 0 }">
		...
		<ul>
			<c:if test="${pageInfo.startPage > pageInfo.pageBlock }">
				<li>
					<a href="BookList.book?type=L&bk2=${bk2 }&page=${pageInfo.startPage-pageInfo.pageBlock}">&lsaquo;</a>
				</li>
			</c:if>
			<li>
				<c:forEach var="i" begin="${pageInfo.startPage }" end="${pageInfo.endPage }" step="1">
					<c:choose>
	                	...
	                	<c:otherwise><a href="BookList.book?type=L&bk2=${bk2 }&page=${i}" class="colorB" style="color: #2a84b6; border-color: #5e9bd1;">${i}</a></c:otherwise>
					</c:choose>
				</c:forEach>
			</li>
			<c:if test="${pageInfo.endPage < pageInfo.maxPage }">
				<li>
					<a href='<c:url value="BookList.book?type=L&bk2=${bk2 }&page=${pageInfo.startPage + pageInfo.pageBlock}"/>'>&rsaquo;</a>
				</li>
			</c:if>
		</ul>
		</div>
	...
	<c:otherwise>
		...
		<ul>
			<c:if test="${pageInfo.startPage > pageInfo.pageBlock }">
				<li>
					<a href="BookList.book?type=L&bk2=${bk2 }&BKID=${book.bookKategorie_BKID}&page=${pageInfo.startPage-pageInfo.pageBlock}">&lsaquo;</a>
				</li>
			</c:if>
			<li>
				<c:forEach var="i" begin="${pageInfo.startPage }" end="${pageInfo.endPage }" step="1">
					<c:choose>
						<c:when test="${i eq pageInfo.page }"><a href="#">${i}</a></c:when>
	                	<c:otherwise><a href="BookList.book?type=L&bk2=${bk2 }&BKID=${book.bookKategorie_BKID}&page=${i}" class="colorB" style="color: #2a84b6; border-color: #5e9bd1;">${i}</a></c:otherwise>
					</c:choose>
				</c:forEach>
			</li>
			<c:if test="${pageInfo.endPage < pageInfo.maxPage }">
				<li>
					<a href='<c:url value="BookList.book?type=L&bk2=${bk2 }&BKID=${book.bookKategorie_BKID}&page=${pageInfo.startPage + pageInfo.pageBlock}"/>'>&rsaquo;</a>
				</li>
			</c:if>
		</ul>
		</div>
